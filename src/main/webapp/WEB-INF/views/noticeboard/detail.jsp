...
<meta charset="UTF-8">
<title>${noticeBoard.noticeBoardTitle }</title>
</head>
<body>
    <h2>공지글 보기</h2>
    <div>
        <p>글 번호 : ${noticeBoard.noticeBoardId }</p>
    </div>
    <div>
        <p>제목 : </p>
        <p>${noticeBoard.noticeBoardTitle }</p>
    </div>
    <div>
        <p>작성자 : ${noticeBoard.memberId }</p>
        <!-- boardDateCreated 데이터 포멧 변경 -->
        <fmt:formatDate value="${noticeBoard.noticeBoardCreatedDate }"
                    pattern="yyyy-MM-dd HH:mm:ss" var="noticeBoardDateCreated"/>
        <p>작성일 : ${noticeBoardDateCreated }</p>
    </div>
    <div>
        <textarea rows="20" cols="120" readonly>${noticeBoard.noticeBoardContent }</textarea>
    </div>
